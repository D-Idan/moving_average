...
def emvwap_strategy_with_reset(
    ...
    def strategy(data_s, alfa_short=alfa_short, alfa_long=alfa_long, short_window=short_window, long_window=long_window,
        ...
        def calculate_em_vwap(span, volume_power=100, day_price="Open", start_index=0):
            ...
            ewma_price_volume = price_volume.ewm(span=span, adjust=False, min_periods=0).mean()
            ewma_volume = data_calc["Volume"].ewm(span=span, adjust=False, min_periods=0).mean()
        ...
        # Initialize the short-term EMVWAP
        EMVWAP_Short_all = pd.Series(np.nan, index=data_s.index)

        # Compute short-term EMVWAP in shifts
        for i in range(0, len(data_s), short_window // 2):  # Overlapping windows
            EMVWAP_Short = calculate_em_vwap(short_window, volume_power=volume_power_short, day_price="Open", start_index=i)
            EMVWAP_Short_all.iloc[i: i + len(EMVWAP_Short)] = EMVWAP_Short.values  # Properly assign values



        EMVWAP_Short = EMVWAP_Short_all
...
if __name__ == "__main__":
    ...
    # Run the backtest
    results = backtester.run(emvwap_strategy_with_reset(short_window=500, long_window=15, reset_window=10))
